...
sudoku_field_t swap_rows_small(sudoku_field_t *source) {
    sudoku_field_t result = *source;
    int region_num = random(REGION_SIDE_LENGTH);
    int first_region_row_num = random(REGION_SIDE_LENGTH);
    int first_row_num = region_num * REGION_SIDE_LENGTH + first_region_row_num;

    int second_region_row_num = random(REGION_SIDE_LENGTH);
    while (first_region_row_num == second_region_row_num) {
        second_region_row_num = random(REGION_SIDE_LENGTH);
    }
...
void remove_cell(sudoku_field_t* source) {
    ...
    int row_num = random(FIELD_SIDE_LENGTH);
    int col_num = random(FIELD_SIDE_LENGTH);

    // todo lock
    while (source->cells[row_num][col_num].value == SUDOKU_EMPTY) {
        row_num = random(FIELD_SIDE_LENGTH);
        col_num = random(FIELD_SIDE_LENGTH);
    }
